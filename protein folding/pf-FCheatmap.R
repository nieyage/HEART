...
#write.table(count,"PF-FCmatrix-FB.txt")
...
dev.off();
pdf("protein_folding-heatmap_for3celltypes.pdf")
pheatmap(pfcount[,1:13],cluster_cols = F,cluster_rows = T,
         color = colorRampPalette(c("navy", "white", "firebrick3"))(100),
         cellwidth = 10, cellheight = 4,
         annotation_col = annotation_col,
         annotation_colors = ann_colors,
...
######单独画每个细胞类型######
pdf("protein_folding-heatmap_forFB.pdf")
pheatmap(pfcount[,1:4],cluster_cols = F,cluster_rows = T,
         color = colorRampPalette(c("navy", "white", "firebrick3"))(100),
         cellwidth = 10, cellheight = 4,
         annotation_col = annotation_col,
         annotation_colors = ann_colors,
         ...
         show_rownames=F,show_colnames=T)
dev.off();
pdf("protein_folding-heatmap_forMP.pdf")
pheatmap(pfcount[,5:8],cluster_cols = F,cluster_rows = T,
         color = colorRampPalette(c("navy", "white", "firebrick3"))(100),
         cellwidth = 10, cellheight = 4,
         annotation_col = annotation_col,
         annotation_colors = ann_colors,
         ...
         show_rownames=F,show_colnames=T)
dev.off();
pdf("protein_folding-heatmap_forEC.pdf")
pheatmap(pfcount[,9:13],cluster_cols = F,cluster_rows = T,
         color = colorRampPalette(c("navy", "white", "firebrick3"))(100),
         cellwidth = 10, cellheight = 4,
         annotation_col = annotation_col,
         annotation_colors = ann_colors,
...
pdf("protein_folding-heatmap_forSMC.pdf")
pheatmap(pfcount[,14:17],cluster_cols = F,cluster_rows = T,
         color = colorRampPalette(c("navy", "white", "firebrick3"))(100),
         cellwidth = 10, cellheight = 4,
         annotation_col = annotation_col,
         annotation_colors = ann_colors,
...
m<-setdiff(order,mp)
order<-c(order,m)
pdf("protein_folding-heatmap_forORDER.pdf")
pheatmap(pfcount[p,],cluster_cols = F,cluster_rows = F,
         color = colorRampPalette(c("navy", "white", "firebrick3"))(100),
         cellwidth = 10, cellheight = 4,
         annotation_col = annotation_col,
         annotation_colors = ann_colors,
...
#######only subtype########
pdf("protein_folding-heatmap_forsubtype.pdf")
pheatmap(pfcount[overlap,c(3,7,12)],cluster_cols = F,cluster_rows = T,
...
FB3count=t(scale(t(count[,7:9]),scale = T,center = T))
UPR<-c("Atf6","Mbtps1","Mbtps2","Wfs1","Map3k5","Bak1","Bax","Capn1","Capn2","Casp12","Ern1","Mapk8",
	"Map2k7","Traf2","Xbp1","Atf4","Eif2ak1","Eif2ak2","Eif2ak4","Ppp1r15a","Nfe2l2","Eif2ak3","Bcl2","Ddit3")
ERAD<-c("Bcap31","Stub1","Cul1","Derl2","Derl3","Derl1","Plaa","Ubqln1","Ubqln2","Ubqln3","Ubqln4","Edem1",
	"Edem2","Edem3","Man1a","Man1a2","Man1b1","Man1c1","Ero1a","Ero1b","Amfr","Herpud1","Syvn1","Dnaja1",
	"Dnaja2","Dnajb1","Dnajb11","Dnajb12","Dnajb14","Dnajb2","Dnajc1","Dnajc10","Dnajc3","Dnajc5","Dnajc5b",
	"Dnajc5g","Hspa1a","Hspa1b","Hspa1l","Hspa2","Hspa4l","Hspa8","Hsp90aa1","Hsp90ab1","Hsp90b1","Nploc4",
	"Os9","Vcp","Prkn","Pdia4","Pdia6","Ngly1","Rad23a","Rad23b","Rbx1","Rnf5","Sec61a1","Sec61a2","Sec61b",
	"Sec61g","Sel1l","Sel1l2","Skp1","Svip","Tram1","Ssr1","Ssr2","Ssr3","Ssr4","Ube2j1","Ube2j2","Ube2g1",
	"Ube2g2","Ube2d1","Ube2d2a","Ube2d2b","Ube2d3","Ube2dnl1","Ube2dnl2","Ubxn1","Ubxn2a","Ubxn4","Ubxn6",
	"Ubxn8","Ufd1","Ube4b","Selenos","Erlec1")

UPRcount<-FB3count[which(rownames(FBcount)%in%UPR),]
ERADcount<-FB3count[which(rownames(FBcount)%in%ERAD),]

pathway<-read.csv("Pathway-gene-transform.csv",header=F)
colnames(pathway)<-c("gene","other","pathway","dec")
pathway<-pathway[which(pathway$gene %in% rownames(UPRcount)),]
annotation_row = data.frame(progress=pathway$pathway)
rownames(annotation_row) = pathway$gene
ann_colors = list(
    progress =c(ATF6="#E64B35B2", IRE1="#4DBBD5B2" ,PERK="#00A087B2", "PERK/ATF6"="#3C5488B2")
)

pdf("FB3-heatmap-UPR.pdf")
pheatmap(UPRcount,cluster_cols = F,cluster_rows = T,
         color = colorRampPalette(c("navy", "white", "firebrick3"))(100),
         cellwidth = 10, cellheight = 10,
         #annotation_col = annotation_col,
         annotation_colors = ann_colors,
...
getAvailableMatrices(ArchRProj = SMC1proj)
SMC1proj <- addMotifAnnotations(ArchRProj = SMC1proj, motifSet = "homer", name = "SMC_Motif",force = TRUE)
markersMotifs <- getMarkerFeatures(ArchRProj = SMC1proj, useMatrix = "MotifMatrix", 
                                  groupBy = "SMC1proj$subtype",  testMethod = "wilcoxon")
markersMotifs
markerList <- getMarkers(markersMotifs,  cutOff = "FDR <= 0.5 & Log2FC >= 0.5")
markerList$SMC1
write.table(markerList,"./matrix/markerList-Motif-Marker-SMC-Subtype-FDR05log2FC05.txt",quote = F,sep = "\t")